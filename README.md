...
| + | [dfdetect — deepfake / synthetic-face detection](project-cv-deepfake/) | ✅ complete | CV learning survey, domain 1/8. Controlled 5-model head-to-head (EfficientNet-B0/ResNet50/ConvNeXt-Tiny/Xception fine-tuned + a CLIP ViT-B/32 linear probe) on real data spanning 40 deepfake generation techniques. Clean accuracy barely differentiated the 4 CNNs (0.994-0.996 ROC-AUC) — so a robustness eval (JPEG/blur/downscale/noise, substituting for an infeasible leave-one-generator-out test) became the real signal: the clean-accuracy "winner" collapses under noise while a statistically-tied model barely moves, flipping which model actually deserves to be served. Also tested and reported that CLIP's web-scale features *don't* transfer well here as a bare linear probe — the honest result over the more exciting-sounding hypothesis. ONNX-served, Dockerized, CI |
